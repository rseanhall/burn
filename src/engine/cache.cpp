    ...
    )
{
    ...
    {
        hr = MemEnsureArraySize(reinterpret_cast<LPVOID*>(&rgSearchPaths), cSearchPaths + 1, sizeof(LPWSTR), cMaxSearchPaths);
        ExitOnFailure(hr, "Failed to ensure size for search paths array.");

        hr = StrAllocString(&rgSearchPaths[cSearchPaths++], wzSourcePath, 0);
    ...
    hr = StrAllocString(&rgSearchPaths[cSearchPaths++], wzDestinationPath, 0);
    ExitOnFailure(hr, "Failed to copy absolute source path.");
    ...
    if (!fSourceIsAbsolute)
    {
        ...
        {
            rgSearchPaths[cSearchPaths] = sczCurrentPath;
            ++cSearchPaths;
            sczCurrentPath = NULL;
        }
        ...
        if (fTryLastFolder)
        {
            ...
            hr = PathConcat(sczLastSourceFolder, wzSourcePath, &rgSearchPaths[cSearchPaths++]);
        ...
        if (wzLayoutDirectory)
        {
            ...
            hr = PathConcat(wzLayoutDirectory, wzSourcePath, &rgSearchPaths[cSearchPaths++]);
    ...
    if (fTryRelativePath)
    {
        ...
        if (fTryLastFolder)
        {
            ...
            hr = PathConcat(sczLastSourceFolder, wzRelativePath, &rgSearchPaths[cSearchPaths++]);
...
LExit:
    ...
    AssertSz(cSearchPaths <= cMaxSearchPaths, "Got more than 7 search paths");
