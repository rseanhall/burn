...
static DWORD CompleteCacheProgress(
    ...
    )
{
    ...
    do
    {
        ...
        for (;;)
        {
            ...
            if (SUCCEEDED(hr))
            {
                CompleteCacheProgress(&progress, qwBundleSize);
                if (progress.fCancel)
                {
    ...
    )
{
    ...
    *pfRetry = FALSE;
    ...
    case BOOTSTRAPPER_CACHE_OPERATION_COPY:
        ...
        if (CSTR_EQUAL != nEquivalentPaths)
        {
            hr = CopyPayload(pProgress, pContext->rgSearchPaths[dwChosenSearchPath], wzDestinationPath);
    ...
    CompleteCacheProgress(pProgress, pContainer ? pContainer->qwFileSize : pPayload->qwFileSize);
    if (pProgress->fCancel)
    {
        ExitFunction1(hr = HRESULT_FROM_WIN32(ERROR_INSTALL_USEREXIT));
    }
    else if (pProgress->fError)
    {
        ExitFunction1(hr = HRESULT_FROM_WIN32(ERROR_INSTALL_FAILURE));
    }
